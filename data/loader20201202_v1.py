...
class DataLoader(object):
    ...
    def __init__(self, filename, batch_size, opt, vocab, evaluation=False):
        ...
        self.raw_data = data
        data,maxlen = self.preprocess(data, vocab, opt)
        self.maxlen=maxlen
    ...
    def preprocess(self, data, vocab, opt):
        ...
        for d in data:
            ...
            max_len=max(max_length,max_len)
            ...
            print(max_length)
            for j,dep_relation in enumerate(deprel):
                if j >= max_length:
                    break
                #-1表示到自己
                token1_id, token2_id = int(head[j]),j+1
                if token2_id == -1 or token2_id >= max_length:
                    token2_id = token1_id
                print("index:{},{}".format(token1_id, token2_id))
                curr_dep[token1_id, token2_id], curr_dep[token2_id, token1_id] = int(dep_relation), int(dep_relation)
            curr_dep=curr_dep.tolist()
            print(curr_dep)
            #print("cur_dep:{}".format(curr_dep))
            '''
    ...
    def __getitem__(self, key):
        ...
        #将实体向量添加到batch
        subj_vec=get_float_tensor(batch[9],batch_size,maxlen)
        obj_vec=get_float_tensor(batch[10],batch_size,maxlen)
        subj_pvec=get_pvec_tensor(batch[11],batch_size,maxlen)
        obj_pvec=get_pvec_tensor(batch[12],batch_size,maxlen)
...
def get_long_tensor(tokens_list, batch_size,maxlen):
    ...
    return tokens
def get_float_tensor(tokens_list, batch_size,maxlen):
    """ Convert list of list of tokens to a padded LongTensor. """
    dim=300
    ...
    return tokens
def get_pvec_tensor(tokens_list, batch_size,maxlen):
    """ Convert list of list of tokens to a padded LongTensor. """
    dim=300
